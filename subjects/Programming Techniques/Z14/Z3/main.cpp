...
class Liga {
	...
	public:
		...
		/// SPREMI I UCITAJ IZ DATOTEKE
		void SacuvajStanje(std::string datoteka) const;
		void AzurirajIzDatoteke(std::string datoteka);
		explicit Liga(std::string datoteka);
};
// implementation of file read/write functions
void Liga::SacuvajStanje(std::string datoteka) const {
	std::ofstream izlazni_tok(datoteka, std::ios::binary);
...
void Liga::AzurirajIzDatoteke(std::string datoteka) {
	std::ifstream ulazni_tok(datoteka, std::ios::in);
	if(!ulazni_tok) throw std::logic_error("Datoteka ne postoji");
	/// start reading data from file
	
	char ime_prvog_tima[20], ime_drugog_tima[20], delim;
	int score1, score2;
	for(;;) {
		ulazni_tok.getline(ime_prvog_tima, sizeof(ime_prvog_tima), '\n');
		ulazni_tok.getline(ime_drugog_tima, sizeof(ime_drugog_tima), '\n');
		ulazni_tok >> score1 >> delim >> score2;
		
		if(ulazni_tok.bad() || delim != ':') 
			throw std::logic_error("Problemi pri citanju datoteke");
		
		/*try { this->timPostoji(ime_prvog_tima, ""); }
		catch (std::exception& e) { this->DodajNoviTim(ime_prvog_tima); }
		
		try { this->timPostoji(ime_drugog_tima, ""); }
		catch (std::exception& e) { this->DodajNoviTim(ime_drugog_tima); }
		*/
		RegistrirajUtakmicu(ime_prvog_tima, ime_drugog_tima, score1, score2);
		if(ulazni_tok.get() == EOF) break;
	}
}
Liga::Liga(std::string datoteka) : max_br_timova(0) {
	std::ifstream ulazni_tok(datoteka, std::ios::binary | std::ios::in);
	if(!ulazni_tok) throw std::logic_error("Datoteka ne postoji");
	...
	Tim temp_team("Temp name");
	for(int i(0); i < broj_timova; ++i) {
		ulazni_tok.read(reinterpret_cast<char*>(&temp_team), sizeof(Tim));
		timovi[i] = new Tim(temp_team);
	}
